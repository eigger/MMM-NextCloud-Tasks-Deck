...
function beautifyList(rawList, showPrefix) {
    let list = [];
    for (parent of rawList)
    {
        if (!showPrefix)
        {
            //console.log(nested);
            parent['summary'] = parent['summary'].replace('List : ', '');
        }
        if (typeof parent.children === "undefined") continue;
        list.push(parent);
    }

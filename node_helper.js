/* Magic Mirror
 * Node Helper: MMM-NextCloud-Tasks-Deck
 *
 * By Jan Ryklikas
 * MIT Licensed.
 */
...
const { transformData, sortList, beautifyList } = require("./transformer");
...
module.exports = NodeHelper.create({
	socketNotificationReceived: function(notification, payload) {
		...
		if (notification === "MMM-NextCloud-Tasks-Deck-UPDATE") {
	...
	getData: async function(moduleId, config, callback) {
		...
		try {
			...
			const beautifedList = beautifyList(nestedList, config.showListPrefix);
			callback(beautifedList);
		} catch (error) {
	...
	sendData: function(moduleId, payload) {
		this.sendSocketNotification("MMM-NextCloud-Tasks-Deck-Helper-TODOS#" + moduleId, payload);
	},

	sendLog: function(moduleId, payload) {
		this.sendSocketNotification("MMM-NextCloud-Tasks-Deck-Helper-LOG#" + moduleId, payload);
	},

	sendError: function(moduleId, payload) {
		this.sendSocketNotification("MMM-NextCloud-Tasks-Deck-Helper-ERROR#" + moduleId, payload);
	}
});
